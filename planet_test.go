...
func TestEV(t *testing.T) {
    ...
    bEarth := Planet{"earth", big.NewFloat(5.97e24), big.NewFloat(6.371e6), big.NewFloat(0.0)}
    if v := bEarth.calcBigEV(); v != big.NewFloat(11.180501) {
        t.Errorf("Issue with escape velocity please check equation: '%f'", v)
    }

    t.Log("Running escape velocity tests for Planet X")
    bNibiru := Planet{"nibiru", big.NewFloat(4.0e22), big.NewFloat(6.21e6), big.NewFloat(0.0)}
    if v := bNibiru.calcBigEV(); v != big.NewFloat(0.926962) {
        t.Errorf("Issue with escape velocity please check equation: '%f'", v)
    }

    t.Log("Running escape velocity tests for Mars")
    bMars := Planet{"mars", big.NewFloat(6.39e23), big.NewFloat(3.4e6), big.NewFloat(0.0)}
    if v := bMars.calcBigEV(); v != big.NewFloat(5.007130) {
        t.Errorf("Issue with escape velocity please check equation: '%f'", v)
    }

    t.Log("Running escape velocity tests for Moon")
    bMoon := Planet{"moon", big.NewFloat(7.35e22), big.NewFloat(1.738e6), big.NewFloat(0.0)}
...
func TestConverter(t *testing.T) {
    ...
    value := Units{"miles_to_ls", big.NewFloat(1000), 5}
...
func TestOrbitor(t *testing.T) {
    ...
    value := Planet{"mars", big.NewFloat(6.39e23), big.NewFloat(3.4e6), big.NewFloat(100.0)}
